...
private func callListNeighbors(item: ListViewItem, topItem: ListViewItem?, bottomItem: ListViewItem?) -> ItemListNeighbors {
    ...
    if let topItem = topItem {
        if let item = item as? ItemListItem, let topItem = topItem as? ItemListItem {
            ...
            if item is TestTaskCallListGroupCallItem && topItem is TestTaskCallListGroupCallItem {
    ...
    if let bottomItem = bottomItem {
        if let item = item as? ItemListItem, let bottomItem = bottomItem as? ItemListItem {
            ...
            if item is TestTaskCallListGroupCallItem && bottomItem is TestTaskCallListGroupCallItem {
...
// MARK: - Models
class TestTaskCallListGroupCallItem: ListViewItem {
    ...
    init(presentationData: ItemListPresentationData, context: AccountContext, style: ItemListStyle, peer: EnginePeer, isActive: Bool, editing: Bool, interaction: TestTaskCallListNodeInteraction) {
    ...
    func nodeConfiguredForParams(async: @escaping (@escaping () -> Void) -> Void, params: ListViewItemLayoutParams, synchronousLoads: Bool, previousItem: ListViewItem?, nextItem: ListViewItem?, completion: @escaping (ListViewItemNode, @escaping () -> (Signal<Void, NoError>?, (ListViewItemApply) -> Void)) -> Void) {
        async {
            let node = TestTaskCallListGroupCallItemNode()
            let makeLayout = node.asyncLayout()
            let (first, last, firstWithHeader) = TestTaskCallListGroupCallItem.mergeType(item: self, previousItem: previousItem, nextItem: nextItem)
    ...
    func updateNode(async: @escaping (@escaping () -> Void) -> Void, node: @escaping () -> ListViewItemNode, params: ListViewItemLayoutParams, previousItem: ListViewItem?, nextItem: ListViewItem?, animation: ListViewItemUpdateAnimation, completion: @escaping (ListViewItemNodeLayout, @escaping (ListViewItemApply) -> Void) -> Void) {
        Queue.mainQueue().async {
            if let nodeValue = node() as? TestTaskCallListGroupCallItemNode {
    ...
    static func mergeType(item: TestTaskCallListGroupCallItem, previousItem: ListViewItem?, nextItem: ListViewItem?) -> (first: Bool, last: Bool, firstWithHeader: Bool) {
        var first = false
        var last = false
        var firstWithHeader = false
        if let previousItem = previousItem {
            if let header = item.header {
                if let previousItem = previousItem as? TestTaskCallListGroupCallItem {
        ...
        if let nextItem = nextItem {
            if let header = item.header {
                if let nextItem = nextItem as? TestTaskCallListGroupCallItem {
...
// MARK: - Node
class TestTaskCallListGroupCallItemNode: ItemListRevealOptionsItemNode {
    ...
    private var layoutParams: (TestTaskCallListGroupCallItem, ListViewItemLayoutParams, Bool, Bool, Bool)?
    ...
    override func layoutForParams(_ params: ListViewItemLayoutParams, item: ListViewItem, previousItem: ListViewItem?, nextItem: ListViewItem?) {
        if let (item, _, _, _, _) = self.layoutParams {
            let (first, last, firstWithHeader) = TestTaskCallListGroupCallItem.mergeType(item: item, previousItem: previousItem, nextItem: nextItem)
    ...
    func asyncLayout() -> (_ item: TestTaskCallListGroupCallItem, _ params: ListViewItemLayoutParams, _ first: Bool, _ last: Bool, _ firstWithHeader: Bool, _ neighbors: ItemListNeighbors) -> (ListViewItemNodeLayout, (Bool) -> (Signal<Void, NoError>?, (Bool) -> Void)) {
