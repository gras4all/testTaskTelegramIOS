...
private struct CallListNodeListViewTransition {
    let callListView: TestTaskCallListNodeView
...
final class TestTaskCallListNodeInteraction {
...
private func mappedInsertEntries(context: AccountContext, presentationData: ItemListPresentationData, showSettings: Bool, nodeInteraction: TestTaskCallListNodeInteraction, entries: [TestTaskCallListNodeViewTransitionInsertEntry]) -> [ListViewInsertItem] {
    return entries.map { entry -> ListViewInsertItem in
        switch entry.entry {
            ...
            case let .groupCall(peer, _, isActive):
                return ListViewInsertItem(index: entry.index, previousIndex: entry.previousIndex, item: TestTaskCallListGroupCallItem(presentationData: presentationData, context: context, style: showSettings ? .blocks : .plain, peer: peer, isActive: isActive, editing: false, interaction: nodeInteraction), directionHint: entry.directionHint)
            case let .messageEntry(topMessage, messages, _, _, dateTimeFormat, editing, hasActiveRevealControls, displayHeader, _):
                return ListViewInsertItem(index: entry.index, previousIndex: entry.previousIndex, item: TestTaskCallListCallItem(presentationData: presentationData, dateTimeFormat: dateTimeFormat, context: context, style: showSettings ? .blocks : .plain, topMessage: topMessage, messages: messages, editing: editing, revealed: hasActiveRevealControls, displayHeader: displayHeader, interaction: nodeInteraction), directionHint: entry.directionHint)
            case let .holeEntry(_, theme):
                return ListViewInsertItem(index: entry.index, previousIndex: entry.previousIndex, item: TestTaskCallListHoleItem(theme: theme), directionHint: entry.directionHint)
        }
    }
}

private func mappedUpdateEntries(context: AccountContext, presentationData: ItemListPresentationData, showSettings: Bool, nodeInteraction: TestTaskCallListNodeInteraction, entries: [TestTaskCallListNodeViewTransitionUpdateEntry]) -> [ListViewUpdateItem] {
    return entries.map { entry -> ListViewUpdateItem in
        switch entry.entry {
            ...
            case let .groupCall(peer, _, isActive):
                return ListViewUpdateItem(index: entry.index, previousIndex: entry.previousIndex, item: TestTaskCallListGroupCallItem(presentationData: presentationData, context: context, style: showSettings ? .blocks : .plain, peer: peer, isActive: isActive, editing: false, interaction: nodeInteraction), directionHint: entry.directionHint)
            case let .messageEntry(topMessage, messages, _, _, dateTimeFormat, editing, hasActiveRevealControls, displayHeader, _):
                return ListViewUpdateItem(index: entry.index, previousIndex: entry.previousIndex, item: TestTaskCallListCallItem(presentationData: presentationData, dateTimeFormat: dateTimeFormat, context: context, style: showSettings ? .blocks : .plain, topMessage: topMessage, messages: messages, editing: editing, revealed: hasActiveRevealControls, displayHeader: displayHeader, interaction: nodeInteraction), directionHint: entry.directionHint)
            case let .holeEntry(_, theme):
                return ListViewUpdateItem(index: entry.index, previousIndex: entry.previousIndex, item: TestTaskCallListHoleItem(theme: theme), directionHint: entry.directionHint)
        }
    }
}

private func mappedCallListNodeViewListTransition(context: AccountContext, presentationData: ItemListPresentationData, showSettings: Bool, nodeInteraction: TestTaskCallListNodeInteraction, transition: TestTaskCallListNodeViewTransition) -> CallListNodeListViewTransition {
...
private final class CallListOpaqueTransactionState {
    ...
    init(callListView: TestTaskCallListNodeView) {
        self.callListView = callListView
    }
}

final class TestTaskCallListControllerNode: ASDisplayNode {
    private weak var controller: TestTaskCallListController?
    ...
    private var callListView: TestTaskCallListNodeView?
    ...
    private var currentLocationAndType = TestTaskCallListNodeLocationAndType(location: .initial(count: 50), scope: .all)
    private let callListLocationAndType = ValuePromise<TestTaskCallListNodeLocationAndType>()
    ...
    init(controller: TestTaskCallListController, context: AccountContext, mode: CallListControllerMode, presentationData: PresentationData, call: @escaping (EnginePeer.Id, Bool) -> Void, joinGroupCall: @escaping (EnginePeer.Id, EngineGroupCallDescription) -> Void, openInfo: @escaping (EnginePeer.Id, [EngineMessage]) -> Void, emptyStateUpdated: @escaping (Bool) -> Void) {
        ...
        let nodeInteraction = TestTaskCallListNodeInteraction(setMessageIdWithRevealedOptions: { [weak self] messageId, fromMessageId in
            if let strongSelf = self {
        ...
        let previousView = Atomic<TestTaskCallListNodeView?>(value: nil)
        ...
        |> mapToQueue { (updateAndType, state, groupCalls, showCallsTab, currentGroupCallPeerId) -> Signal<CallListNodeListViewTransition, NoError> in
            ...
            let processedView = TestTaskCallListNodeView(originalView: update.view, filteredEntries: callListNodeEntriesForView(view: update.view, groupCalls: groupCalls, state: state, showSettings: showSettings, showCallsTab: showCallsTab, isRecentCalls: type == .all, currentGroupCallPeerId: currentGroupCallPeerId), presentationData: state.presentationData)
            let previous = previousView.swap(processedView)
            
            let reason: TestTaskCallListNodeViewTransitionReason
        ...
        self.listNode.displayedItemRangeChanged = { [weak self] range, transactionOpaqueState in
            if let strongSelf = self, let range = range.loadedRange, let view = (transactionOpaqueState as? CallListOpaqueTransactionState)?.callListView.originalView {
                var location: TestTaskCallListNodeLocation?
                ...
                if let location = location, location != strongSelf.currentLocationAndType.location {
                    strongSelf.currentLocationAndType = TestTaskCallListNodeLocationAndType(location: location, scope: strongSelf.currentLocationAndType.scope)
    ...
    func updateType(_ type: EngineCallList.Scope) {
        if type != self.currentLocationAndType.scope {
            if let view = self.callListView?.originalView {
                ...
                self.currentLocationAndType = TestTaskCallListNodeLocationAndType(location: .changeType(index: index), scope: type)
    ...
    func scrollToLatest() {
        if let view = self.callListView?.originalView, !view.hasLater {
            ...
            let location: TestTaskCallListNodeLocation = .scroll(index: EngineMessage.Index.absoluteUpperBound(), sourceIndex: EngineMessage.Index.absoluteLowerBound(), scrollPosition: .top(0.0), animated: true)
            self.currentLocationAndType = TestTaskCallListNodeLocationAndType(location: location, scope: self.currentLocationAndType.scope)
            self.callListLocationAndType.set(self.currentLocationAndType)
        }
    }
