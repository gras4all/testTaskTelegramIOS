...
private func callListNeighbors(item: ListViewItem, topItem: ListViewItem?, bottomItem: ListViewItem?) -> ItemListNeighbors {
    ...
    if let topItem = topItem {
        if let item = item as? ItemListItem, let topItem = topItem as? ItemListItem {
            ...
            if item is TestTaskCallListCallItem && topItem is TestTaskCallListCallItem {
    ...
    if let bottomItem = bottomItem {
        if let item = item as? ItemListItem, let bottomItem = bottomItem as? ItemListItem {
            ...
            if item is TestTaskCallListCallItem && bottomItem is TestTaskCallListCallItem {
...
// MARK: - Models
final class TestTaskCallListCallItem: ListViewItem {
    ...
    let editing: Bool
    let revealed: Bool
    let interaction: TestTaskCallListNodeInteraction
    
    let selectable: Bool = true
    let headerAccessoryItem: ListViewAccessoryItem?
    let header: ListViewItemHeader?
    
    init(presentationData: ItemListPresentationData, dateTimeFormat: PresentationDateTimeFormat, context: AccountContext, style: ItemListStyle, topMessage: EngineMessage, messages: [EngineMessage], editing: Bool, revealed: Bool, displayHeader: Bool, interaction: TestTaskCallListNodeInteraction) {
    ...
    func nodeConfiguredForParams(async: @escaping (@escaping () -> Void) -> Void, params: ListViewItemLayoutParams, synchronousLoads: Bool, previousItem: ListViewItem?, nextItem: ListViewItem?, completion: @escaping (ListViewItemNode, @escaping () -> (Signal<Void, NoError>?, (ListViewItemApply) -> Void)) -> Void) {
        async {
            let node = TestTaskCallListCallItemNode()
            let makeLayout = node.asyncLayout()
            let (first, last, firstWithHeader) = TestTaskCallListCallItem.mergeType(item: self, previousItem: previousItem, nextItem: nextItem)
    ...
    func updateNode(async: @escaping (@escaping () -> Void) -> Void, node: @escaping () -> ListViewItemNode, params: ListViewItemLayoutParams, previousItem: ListViewItem?, nextItem: ListViewItem?, animation: ListViewItemUpdateAnimation, completion: @escaping (ListViewItemNodeLayout, @escaping (ListViewItemApply) -> Void) -> Void) {
        Queue.mainQueue().async {
            if let nodeValue = node() as? TestTaskCallListCallItemNode {
    ...
    static func mergeType(item: TestTaskCallListCallItem, previousItem: ListViewItem?, nextItem: ListViewItem?) -> (first: Bool, last: Bool, firstWithHeader: Bool) {
        var first = false
        var last = false
        var firstWithHeader = false
        if let previousItem = previousItem {
            if let header = item.header {
                if let previousItem = previousItem as? TestTaskCallListCallItem {
        ...
        if let nextItem = nextItem {
            if let header = item.header {
                if let nextItem = nextItem as? TestTaskCallListCallItem {
...
// MARK: - Nodes
final class TestTaskCallListCallItemNode: ItemListRevealOptionsItemNode {
    ...
    private var layoutParams: (TestTaskCallListCallItem, ListViewItemLayoutParams, Bool, Bool, Bool)?
    ...
    override func layoutForParams(_ params: ListViewItemLayoutParams, item: ListViewItem, previousItem: ListViewItem?, nextItem: ListViewItem?) {
        if let (item, _, _, _, _) = self.layoutParams {
            let (first, last, firstWithHeader) = TestTaskCallListCallItem.mergeType(item: item, previousItem: previousItem, nextItem: nextItem)
    ...
    func asyncLayout() -> (_ item: TestTaskCallListCallItem, _ params: ListViewItemLayoutParams, _ first: Bool, _ last: Bool, _ firstWithHeader: Bool, _ neighbors: ItemListNeighbors) -> (ListViewItemNodeLayout, (Bool) -> (Signal<Void, NoError>?, (Bool) -> Void)) {
    ...
    override func updateRevealOffset(offset: CGFloat, transition: ContainedViewLayoutTransition) {
        ...
        if let (item, params, _, _, _) = self.layoutParams {
            ...
            transition.updateFrameAdditive(
                node: self.dateNode, frame: CGRect(origin: CGPoint(x: editingOffset + revealOffset + self.bounds.size.width - dateRightInset - self.dateNode.bounds.size.width, y: self.dateNode.frame.minY), size: self.dateNode.bounds.size))
