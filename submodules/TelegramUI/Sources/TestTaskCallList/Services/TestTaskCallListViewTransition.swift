...
struct TestTaskCallListNodeView {
    let originalView: EngineCallList
    let filteredEntries: [TestTaskCallListNodeEntry]
    let presentationData: ItemListPresentationData
}

enum TestTaskCallListNodeViewTransitionReason {
    case initial
...
struct TestTaskCallListNodeViewTransitionInsertEntry {
    let index: Int
    let previousIndex: Int?
    let entry: TestTaskCallListNodeEntry
    let directionHint: ListViewItemOperationDirectionHint?
}

struct TestTaskCallListNodeViewTransitionUpdateEntry {
    let index: Int
    let previousIndex: Int
    let entry: TestTaskCallListNodeEntry
    let directionHint: ListViewItemOperationDirectionHint?
...
struct TestTaskCallListNodeViewTransition {
    let callListView: TestTaskCallListNodeView
    let deleteItems: [ListViewDeleteItem]
    let insertEntries: [TestTaskCallListNodeViewTransitionInsertEntry]
    let updateEntries: [TestTaskCallListNodeViewTransitionUpdateEntry]
    let options: ListViewDeleteAndInsertOptions
    let scrollToItem: ListViewScrollToItem?
    let stationaryItemRange: (Int, Int)?
}

enum TestTaskCallListNodeViewScrollPosition {
...
func preparedCallListNodeViewTransition(from fromView: TestTaskCallListNodeView?, to toView: TestTaskCallListNodeView, reason: TestTaskCallListNodeViewTransitionReason, disableAnimations: Bool, context: AccountContext, scrollPosition: TestTaskCallListNodeViewScrollPosition?) -> Signal<TestTaskCallListNodeViewTransition, NoError> {
    return Signal { subscriber in
        ...
        var adjustedIndicesAndItems: [TestTaskCallListNodeViewTransitionInsertEntry] = []
        var adjustedUpdateItems: [TestTaskCallListNodeViewTransitionUpdateEntry] = []
        ...
        for (index, entry, previousIndex) in indicesAndItems {
            ...
            adjustedIndicesAndItems.append(TestTaskCallListNodeViewTransitionInsertEntry(index: adjustedIndex, previousIndex: adjustedPrevousIndex, entry: entry, directionHint: directionHint))
        }
        ...
        for (index, entry, previousIndex) in updateIndices {
            ...
            adjustedUpdateItems.append(TestTaskCallListNodeViewTransitionUpdateEntry(index: adjustedIndex, previousIndex: adjustedPreviousIndex, entry: entry, directionHint: directionHint))
        }
        ...
        subscriber.putNext(TestTaskCallListNodeViewTransition(callListView: toView, deleteItems: adjustedDeleteIndices, insertEntries: adjustedIndicesAndItems, updateEntries: adjustedUpdateItems, options: options, scrollToItem: scrollToItem, stationaryItemRange: stationaryItemRange))
