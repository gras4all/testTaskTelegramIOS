                    ...
                    switch navigation {
                        case .info:
                            ...
                            |> deliverOnMainQueue).start(next: { peer in
                                if let infoController = context.sharedContext.makePeerInfoController(context: context, updatedPresentationData: nil, peer: peer, mode: .generic, avatarInitiallyExpanded: false, fromChat: false, requestsContext: nil, isTestTask: false) {
                ...
                if parsedUrl.host == "localpeer" {
                    ...
                    if let components = URLComponents(string: "/?" + query) {
                        ...
                        if let id = id, !id.isEmpty, let idValue = Int64(id), idValue > 0 {
                            ...
                            |> deliverOnMainQueue).start(next: { peer in
                                if let peer = peer, let controller = context.sharedContext.makePeerInfoController(context: context, updatedPresentationData: nil, peer: peer._asPeer(), mode: .generic, avatarInitiallyExpanded: false, fromChat: false, requestsContext: nil, isTestTask: false) {
