...
final class ChatRecentActionsControllerNode: ViewControllerTracingNode {
    ...
    private func openPeerMention(_ name: String) {
        ...
        |> deliverOnMainQueue).start(next: { [weak self] peer in
            if let strongSelf = self {
                if let peer = peer {
                    if let infoController = strongSelf.context.sharedContext.makePeerInfoController(context: strongSelf.context, updatedPresentationData: nil, peer: peer._asPeer(), mode: .generic, avatarInitiallyExpanded: false, fromChat: false, requestsContext: nil, isTestTask: false) {
                        strongSelf.pushController(infoController)
                    }
                }
            }
