...
final class TestTaskAccountScreenNode: ViewControllerTracingNode, UIScrollViewDelegate {
    private weak var controller: TestTaskAccountScreenImpl?
    ...
    private var regularSections: [AnyHashable: TestTaskAccountScreenItemSectionContainerNode] = [:]
    private var editingSections: [AnyHashable: TestTaskAccountScreenItemSectionContainerNode] = [:]
    ...
    private var _interaction: TestTaskAccountInteraction?
    private var interaction: TestTaskAccountInteraction {
    ...
    init(controller: TestTaskAccountScreenImpl, context: AccountContext, peerId: PeerId, avatarInitiallyExpanded: Bool, isOpenedFromChat: Bool, nearbyPeerDistance: Int32?, callMessages: [Message], isSettings: Bool, hintGroupInCommon: PeerId?, requestsContext: PeerInvitationImportersContext?) {
        ...
        self._interaction = TestTaskAccountInteraction(
            ...
            openTestTaskAccount: { [weak self] peer, isMember in
                self?.openTestTaskAccount(peer: peer, isMember: isMember)
            },
            performMemberAction: { [weak self] member, action in
                self?.performMemberAction(member: member, action: action)
            },
            openTestTaskAccountContextMenu: { [weak self] subject, sourceNode in
                self?.openTestTaskAccountContextMenu(subject: subject, sourceNode: sourceNode)
            },
        ...
        self.paneContainerNode.requestPerformPeerMemberAction = { [weak self] member, action in
            ...
            switch action {
            case .open:
                strongSelf.openTestTaskAccount(peer: member.peer, isMember: true)
            case .promote:
                ...
                case .info:
                    ...
                    |> deliverOnMainQueue).start(next: { [weak self] peer in
                        if let strongSelf = self, peer.restrictionText(platform: "ios", contentSettings: strongSelf.context.currentContentSettings.with { $0 }) == nil {
                            if let infoController = strongSelf.context.sharedContext.makePeerInfoController(context: strongSelf.context, updatedPresentationData: nil, peer: peer, mode: .generic, avatarInitiallyExpanded: false, fromChat: false, requestsContext: nil, isTestTask: false) {
    ...
    private func performButtonAction(key: PeerInfoHeaderButtonKey, gesture: ContextGesture?) {
        ...
        case .mute:
            if let notificationSettings = self.data?.notificationSettings, case .muted = notificationSettings.muteState {
                ...
                if let sourceNode = self.headerNode.buttonNodes[.mute]?.referenceNode {
                    let contextController = ContextController(account: self.context.account, presentationData: self.presentationData, source: .reference(TestTaskAccountContextReferenceContentSource(controller: controller, sourceNode: sourceNode)), items: .single(ContextController.Items(content: .list(items))), gesture: gesture)
        ...
        case .more:
            ...
            if let sourceNode = self.headerNode.buttonNodes[.more]?.referenceNode {
                ...
                let contextController = ContextController(account: self.context.account, presentationData: self.presentationData, source: .reference(TestTaskAccountContextReferenceContentSource(controller: controller, sourceNode: sourceNode)), items: items |> map { ContextController.Items(content: .list($0)) }, gesture: gesture)
                contextController.dismissed = { [weak self] in
                    if let strongSelf = self {
                        strongSelf.state = strongSelf.state.withHighlightedButton(nil)
                        if let (layout, navigationHeight) = strongSelf.validLayout {
        ...
        |> map { currentAccountPeer, availablePeers -> [FoundPeer] in
            ...
            if let contextController = contextController {
                ...
                if let sourceNode = strongSelf.headerNode.buttonNodes[.voiceChat]?.referenceNode, let controller = strongSelf.controller {
                    let contextController = ContextController(account: strongSelf.context.account, presentationData: strongSelf.presentationData, source: .reference(TestTaskAccountContextReferenceContentSource(controller: controller, sourceNode: sourceNode)), items: .single(ContextController.Items(content: .list(items))), gesture: gesture)
                    contextController.dismissed = { [weak self] in
                        if let strongSelf = self {
                            strongSelf.state = strongSelf.state.withHighlightedButton(nil)
                            if let (layout, navigationHeight) = strongSelf.validLayout {
    ...
    private func performBotCommand(command: TestTaskAccountBotCommand) {
    ...
    private func openParticipantsSection(section: TestTaskAccountParticipantsSection) {
        guard let data = self.data, let peer = data.peer else {
            return
        }
        switch section {
    ...
    private func openTestTaskAccount(peer: Peer, isMember: Bool) {
        var mode: PeerInfoControllerMode = .generic
        if isMember {
            mode = .group(self.peerId)
        }
        if let infoController = self.context.sharedContext.makePeerInfoController(context: self.context, updatedPresentationData: nil, peer: peer, mode: mode, avatarInitiallyExpanded: false, fromChat: false, requestsContext: nil, isTestTask: false) {
            (self.controller?.navigationController as? NavigationController)?.pushViewController(infoController)
        }
    }
    
    private func performMemberAction(member: PeerInfoMember, action: TestTaskAccountMemberAction) {
        guard let data = self.data, let peer = data.peer else {
            return
        }
        switch action {
    ...
    private func openTestTaskAccountContextMenu(subject: TestTaskAccountContextSubject, sourceNode: ASDisplayNode) {
    ...
    func openSettings(section: TestTaskAccountSettingsSection) {
    ...
    private func displayMediaGalleryContextMenu(source: ContextReferenceContentNode, gesture: ContextGesture?) {
        ...
        |> deliverOnMainQueue).start(next: { [weak self] messageCounts in
            ...
            let contextController = ContextController(account: strongSelf.context.account, presentationData: strongSelf.presentationData, source: .reference(TestTaskAccountContextReferenceContentSource(controller: controller, sourceNode: source)), items: .single(ContextController.Items(content: .list(items))), gesture: gesture)
    ...
    func containerLayoutUpdated(layout: ContainerViewLayout, navigationHeight: CGFloat, transition: ContainedViewLayoutTransition, additive: Bool = false) {
        ...
        if !self.isMediaOnly {
            ...
            for (sectionId, sectionItems) in items {
                ...
                var wasAdded = false
                let sectionNode: TestTaskAccountScreenItemSectionContainerNode
                if let current = self.regularSections[sectionId] {
                    sectionNode = current
                } else {
                    sectionNode = TestTaskAccountScreenItemSectionContainerNode()
    ...
    func updateNavigation(transition: ContainedViewLayoutTransition, additive: Bool) {
...
private final class TestTaskAccountContextReferenceContentSource: ContextReferenceContentSource {
...
//MARK: - settingsItems
private func settingsItems(data: PeerInfoScreenData?, context: AccountContext, presentationData: PresentationData, interaction: TestTaskAccountInteraction, isExpanded: Bool) -> [(AnyHashable, [PeerInfoScreenItem])] {
    ...
    /*items[.payment]!.append(TestTaskAccountScreenDisclosureItem(id: 100, label: .text(""), text: "Payment Method", icon: PresentationResourcesSettings.language, action: {
...
//MARK: - settingsEditingItems
private func settingsEditingItems(data: PeerInfoScreenData?, state: PeerInfoState, context: AccountContext, presentationData: PresentationData, interaction: TestTaskAccountInteraction) -> [(AnyHashable, [PeerInfoScreenItem])] {
    guard let data = data else {
        return []
    }
...
//MARK: - infoItems
private func infoItems(data: PeerInfoScreenData?, context: AccountContext, presentationData: PresentationData, interaction: TestTaskAccountInteraction, nearbyPeerDistance: Int32?, callMessages: [Message]) -> [(AnyHashable, [PeerInfoScreenItem])] {
    guard let data = data else {
        return []
    }
    ...
    let bioContextAction: (ASDisplayNode) -> Void = { sourceNode in
        interaction.openTestTaskAccountContextMenu(.bio, sourceNode)
    }
    ...
    if let user = data.peer as? TelegramUser {
        if !callMessages.isEmpty {
            let callListItem = PeerInfoScreenCallListItem(id: 20, messages: callMessages)
            callListItem.isTestTask = true
            items[.calls]!.append(callListItem)
        }
        ...
        if let phone = user.phone {
            ...
            items[.peerInfo]!.append(PeerInfoScreenLabeledValueItem(id: 2, label: presentationData.strings.ContactInfo_PhoneLabelMobile, text: formattedPhone, textColor: .accent, action: {
                ...
                interaction.openTestTaskAccountContextMenu(.phone(formattedPhone), sourceNode)
            }, requestLayout: {
        ...
        if let username = user.username {
            items[.peerInfo]!.append(PeerInfoScreenLabeledValueItem(id: 1, label: presentationData.strings.Profile_Username, text: "@\(username)", textColor: .accent, icon: .qrCode, action: {
                ...
                interaction.openTestTaskAccountContextMenu(.link, sourceNode)
            }, iconAction: {
    ...
    if let peer = data.peer, let members = data.members, case let .shortList(_, memberList) = members {
        ...
        for member in memberList {
            ...
            items[.peerMembers]!.append(PeerInfoScreenMemberItem(id: member.id, context: context, enclosingPeer: peer, member: member, action: isAccountPeer ? nil : { action in
                switch action {
                case .open:
                    interaction.openTestTaskAccount(member.peer, true)
                case .promote:
...
//MARK: - editingItems
private func editingItems(data: PeerInfoScreenData?, context: AccountContext, presentationData: PresentationData, interaction: TestTaskAccountInteraction) -> [(AnyHashable, [PeerInfoScreenItem])] {
    enum Section: Int, CaseIterable {
        case notifications
        case groupLocation
